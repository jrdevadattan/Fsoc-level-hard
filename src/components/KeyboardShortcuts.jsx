...
export default function KeyboardShortcuts() {
  ...
  const getAnswerButtons = () => {
    const nodes = Array.from(document.querySelectorAll('[data-quiz-answer="true"]'))
  ...
  useEffect(() => {
    const onKeyDown = (e) => {
      ...
      if (e.key === "r" || e.key === "R") {
        const restartBtn = document.querySelector('[data-quiz-restart="true"]')
        if (restartBtn) {
          e.preventDefault()
          restartBtn.click()
        }
  ...
  return showHelp ? (
    ...
    >
      <div style={{ fontWeight: 700, marginBottom: "0.5rem" }}>Keyboard Shortcuts</div>
