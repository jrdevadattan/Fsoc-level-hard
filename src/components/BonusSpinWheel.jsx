...
const BonusSpinWheel = ({ isOpen, onClose, onRewardWon, quizScore }) => {
  ...
  const spinWheel = () => {
    ...
    const offset = segmentAngle / 2; // Center of segment
    const finalRotation = baseRotation + (360 - rewardAngle - offset);
    
    // Force transition by resetting rotation before applying final target
    setRotation((r) => r % 360);
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-gradient-to-br from-gray-900 to-black rounded-3xl shadow-2xl max-w-2xl w-full p-8 relative animate-scale-in">
        {/* Close button */}
        <button
          onClick={handleClose}
          disabled={isSpinning}
        ...
        <div className="relative flex items-center justify-center mb-8">
          ...
          <div className="relative w-80 h-80">
            <div
              className="w-full h-full rounded-full relative overflow-hidden shadow-2xl border-8 border-gray-700"
              style={{
                transform: `rotate(${rotation}deg)`,
                transition: isSpinning ? 'transform 4s cubic-bezier(0.22,0.61,0.36,1)' : 'none'
              }}
            >
            ...
            {[...Array(80)].map((_, i) => (
              <span
                ...
                style={{
                  ...
                  animation: `confetti-fall ${0.8 + Math.random() * 1.2}s linear forwards`,
          ...
          <div className="text-center mb-6 animate-scale-in">
            <div className="bg-gradient-to-r from-purple-600 to-blue-600 rounded-xl p-6 shadow-lg">
              ...
              <p className="text-white text-4xl font-bold">
                {wonReward.isMultiplier ? `${wonReward.label} Score Bonus!` : `${wonReward.label} Points!`}
              </p>
